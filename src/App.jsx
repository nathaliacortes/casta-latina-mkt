...
export default function App() {
  ...
  const sbGet = (table) =>
    fetch(`${SB_URL}/rest/v1/${table}?select=*`, { headers: sbHeaders })
      .then(r => r.json());
  ...
  const sbDelete = (table, id) =>
    fetch(`${SB_URL}/rest/v1/${table}?id=eq.${id}`, { method: "DELETE", headers: sbHeaders });

  // ── Load from Supabase on mount ───────────────────────────────────────
  useEffect(() => {
    Promise.all([sbGet("solicitudes"), sbGet("equipo")])
      .then(([solsData, equipoData]) => {
        if (!solsData?.length && !equipoData?.length) {
          const plain = DEMO_PLAIN();
          Promise.all([
            sbUpsert("solicitudes", plain.map(solToDb)),
            sbUpsert("equipo", EQUIPO_INICIAL),
          ]);
          setSolsRaw(plain);
  ...
  // ── Wrapped setters — sync to Supabase ────────────────────────────────
  const setSols = useCallback((updater) => {
    setSolsRaw(prev => {
      ...
      setTimeout(() => {
        upserted.forEach(s => sbUpsert("solicitudes", solToDb(s)));
        deleted.forEach(id => sbDelete("solicitudes", id));
      }, 0);
      return next;
    });
  }, []);
  ...
  const setEquipo = useCallback((updater) => {
    setEquipoRaw(prev => {
      ...
      setTimeout(() => {
        upserted.forEach(m => sbUpsert("equipo", m));
        deleted.forEach(id => sbDelete("equipo", id));
      }, 0);
      return next;
    });
  }, []);
