    ...
    <div style={{minHeight:"100vh",background:"#F1F5F9",fontFamily:"'Outfit',sans-serif",color:"#1E293B"}}>
      ...
      <main className="plat-main" style={{maxWidth:1280,margin:"0 auto"}}>
        ...
        {usuario.rol==="director"&&solsVis.length>0&&(()=>{
          ...
          const sinSubtareas = solsVis.filter(s => s.subtareas.length === 0 && s.estado !== "cerrada");
          const porDelegar   = solsVis.filter(s => s.subtareas.length > 0 && s.subtareas.some(st=>!st.asignadoId&&["Pendiente","En Producción"].includes(st.estado)));
          const enProceso    = solsVis.filter(s => {
            if(s.subtareas.length === 0) return false;
          ...
          const CATS = [
            { key:"nueva",     label:"Recién Creadas",       icon:"🆕", color:"#3F4462", bg:"#F0F1F8", border:"#C7CAE0", items:sinSubtareas, desc:"Sin subtareas asignadas aún" },
...
export default function App() {
  ...
  const flashSync = useCallback((s) => {
    ...
    // Auto-dismiss both saved and error after a moment
    syncTimer.current = setTimeout(() => setSyncStatus(null), s === "saved" ? 1500 : 3000);
  }, []);
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        // Mark flags as consumed so next real user action writes immediately
        isFirstSols.current   = false;
        isFirstEquipo.current = false;
  ...
  const lastWrite = useRef(0); // timestamp of last local write

  // ── Real-time sync: poll Supabase for remote changes ──────────────────
  useEffect(() => {
    const unsubSols = sb.subscribe("sols", (val) => {
      // Don't overwrite if we wrote within the last 6 seconds (avoid echo)
      if (Date.now() - lastWrite.current < 6000) return;
      setSolsRaw(val);
    });
  ...
  useEffect(() => {
    if (sols === null || !isLoaded.current) return;
  ...
  useEffect(() => {
    if (equipo === null || !isLoaded.current) return;
