...
function TareaCard({st,sol,usuario,equipo=[],onUpd,onDelete,notify}){
  ...
  // esEquipo: true if task belongs to user's area, OR if an area member is assigned (apoyo)
...
function PlatformApp({ sols, setSols, equipo, setEquipo, usuario, setUsuario, onGoHome, pendingDirectorInit=false, clearPendingDirector }){
  ...
  const [filtroPersona,setFiltroPersona]=useState("Todas"); // "Todas" | member id
  ...
  const solsVis=useMemo(()=>{
    ...
    if(usuario.rol==="equipo"){
      // Show solicitudes that have active tasks in this area (assigned OR unassigned)
      list=list.filter(s=>s.subtareas.some(st=>
        st.area===usuario.area &&
        ["Pendiente","En Producción","Cambios Solicitados"].includes(st.estado)
      ));
      if(filtroPersona==="__sin_asignar__") {
        list=list.filter(s=>s.subtareas.some(st=>
          st.area===usuario.area &&
          !st.asignadoId &&
          ["Pendiente","En Producción","Cambios Solicitados"].includes(st.estado)
        ));
      } else if(filtroPersona!=="Todas") {
        list=list.filter(s=>s.subtareas.some(st=>
          st.area===usuario.area &&
          st.asignadoId===filtroPersona &&
          ["Pendiente","En Producción","Cambios Solicitados"].includes(st.estado)
        ));
      }
    }
    ...
    return list;
  },[sols,usuario,filtroTipo,tabVista,filtroPersona]);
    ...
    <div style={{minHeight:"100vh",background:"#F1F5F9",fontFamily:"'Outfit',sans-serif",color:"#1E293B"}}>
      ...
      <main className="plat-main" style={{maxWidth:1280,margin:"0 auto"}}>
        ...
        {usuario.rol==="equipo"&&(()=>{
          ...
          // Build flat task list for a member — only tasks in THIS area
          ...
          // Include ANYONE assigned to a task in this area — own area members + cross-area support
          ...
          // Merge: own area first, then cross-area support
          ...
          const urgencyLabel = (diff) => {
            if(diff===null) return {label:"Sin fecha",color:"#94A3B8",bg:"#F8FAFC",icon:"📌"};
            if(diff<0)  return {label:"Vencida",   color:"#EF4444",bg:"#FEF2F2",icon:"🔴"};
            if(diff===0) return {label:"Vence hoy", color:"#E84B2C",bg:"#FEF3F0",icon:"⚡"};
            if(diff<=2)  return {label:"Esta semana",color:"#F5A623",bg:"#FFFBEB",icon:"⚠️"};
            return {label:"Próximamente",color:"#3BAD6E",bg:"#F0FDF4",icon:"📅"};
          };

          return (
            <div style={{display:"flex",flexDirection:"column",gap:12,marginBottom:20}}>

              {/* Header: área + toggle vista */}
              <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:10}}>
                <div style={{display:"flex",alignItems:"center",gap:10}}>
                  ...
                  <div>
                    <div style={{fontSize:15,fontWeight:800,color:"#0F172A",fontFamily:"Outfit,sans-serif"}}>Área de {usuario.area}</div>
              ...
              {vistaEquipo==="mis-tareas"&&(
                <div style={{display:"flex",flexDirection:"column",gap:10}}>
                    ...
                    <div style={{display:"flex",flexDirection:"column",gap:8}}>
                      ...
                      {tareasActivas.length===0&&(
                        <div style={{padding:"30px 20px",textAlign:"center",background:"#fff",borderRadius:12,border:"2px dashed #E2E8F0"}}>
                          ...
                          <div style={{fontSize:13,fontWeight:700,color:"#2B3A5C"}}>¡Al día!</div>
                      ...
                      {(()=>{
                        ...
                        const grupos = [
                          ...
                          {key:"proximas",   label:"Próximamente",  icon:"📅", color:"#3BAD6E", bg:"#F0FDF4", border:"#6EE7B7", items:proximas},
                        ...
                        return grupos.map(grupo=>(
                          <div key={grupo.key}>
                            ...
                            {grupo.items.map(tarea=>{
                              ...
                              return(
                                <div key={tarea.id}
                                  style={{background:"#fff",borderRadius:10,border:`1px solid ${grupo.border}`,borderLeft:`4px solid ${grupo.color}`,padding:"12px 14px",marginBottom:6,display:"flex",alignItems:"flex-start",gap:12,boxShadow:"0 1px 4px rgba(0,0,0,.04)"}}>
                                  <div style={{width:32,height:32,borderRadius:8,background:areaCfg.bg,border:`1px solid ${areaCfg.border||areaCfg.color+"22"}`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:16,flexShrink:0}}>{areaCfg.icon}</div>
                                  <div style={{flex:1,minWidth:0}}>
                                    <div style={{fontSize:12,fontWeight:700,color:"#0F172A",marginBottom:2}}>{tarea.tipo}</div>
                                    ...
                                    <div style={{display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"}}>
                                      ...
                                      {tarea.estado==="Cambios Solicitados"&&tarea.comentario&&<span style={{fontSize:10,color:"#D97706",fontWeight:600}}>⚠️ "{tarea.comentario}"</span>}
                                    </div>
                                  </div>
                  ...
                  {!personaSeleccionada&&tareasActivas.length===0&&miembrosArea.length>0&&(
                    <div style={{padding:"24px",textAlign:"center",background:"#fff",borderRadius:12,border:"2px dashed #E2E8F0"}}>
                      <div style={{fontSize:24,marginBottom:8}}>👆</div>
                      <div style={{fontSize:13,fontWeight:700,color:"#2B3A5C",marginBottom:4}}>Selecciona tu nombre arriba</div>
        ...
        {solsVis.length===0&&(usuario.rol!=="equipo"||vistaEquipo==="solicitudes")&&(
        ...
        {usuario.rol==="director"&&solsVis.length>0&&(()=>{
          ...
          return CATS.map(cat=>(
            ...
            const misSubtareas=usuario.rol==="equipo"?sol.subtareas.filter(s=>
              s.area===usuario.area &&
              ["Pendiente","En Producción","Cambios Solicitados"].includes(s.estado) &&
              (filtroPersona==="Todas" ||
               (filtroPersona==="__sin_asignar__" && !s.asignadoId) ||
               s.asignadoId===filtroPersona)
            ):[];
