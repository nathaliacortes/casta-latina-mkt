  ...
  return (
    <div style={{position:"fixed",inset:0,background:"rgba(15,23,42,.65)",zIndex:700,display:"flex",alignItems:"center",justifyContent:"center",backdropFilter:"blur(5px)"}}>
      ...
      <div className="modal-config-w" style={{background:"#fff",borderRadius:20,maxHeight:"92dvh",overflow:"hidden",boxShadow:"0 28px 70px rgba(0,0,0,.28)",display:"flex",flexDirection:"column"}}>
        {/* Header */}
        <div style={{padding:"16px 24px",background:"#2B3A5C",borderBottom:"3px solid #E84B2C",flexShrink:0,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          <div>
            <div style={{fontFamily:"Syne,sans-serif",fontSize:17,fontWeight:800,color:"#fff"}}>⚙️ Configuración de Equipo</div>
            <div style={{fontSize:11,color:"rgba(255,255,255,.45)",marginTop:1}}>{equipo.filter(m=>m.activo).length} activos · {equipo.filter(m=>!m.activo).length} inactivos · {equipo.length} total</div>
          </div>
        ...
        <div className="modal-config-body" style={{flex:1,overflow:"hidden",display:"grid"}}>
          {/* LEFT: Agregar nuevo */}
          <div style={{padding:"20px 20px",borderRight:"1px solid #E2E8F0",overflow:"auto",display:"flex",flexDirection:"column",gap:14}}>
            <div style={{padding:"10px 12px",background:"#EEF2FF",border:"1px solid #C7D2FE",borderRadius:10,fontSize:11,color:"#3730A3",fontWeight:600}}>
              ➕ Agregar nuevo integrante
            </div>
            <div><FormLabel label="Nombre completo" req/><input value={nombre} onChange={e=>setNombre(e.target.value)} onKeyDown={e=>e.key==="Enter"&&agregar()} placeholder="Nombre Apellido" style={inp}/></div>
            <div>
              <FormLabel label="Área"/>
              <div style={{display:"flex",flexWrap:"wrap",gap:5,marginTop:4}}>
                {AREAS_PROD.map(a=>{const cfg=AREA_CFG[a]||{};const isA=area===a;return(
                  <button key={a} onClick={()=>setArea(a)}
                    style={{padding:"5px 11px",borderRadius:7,border:`2px solid ${isA?cfg.color:"#E2E8F0"}`,background:isA?cfg.bg||"#F8FAFC":"#fff",color:isA?cfg.color:"#64748B",fontSize:11,fontWeight:isA?700:400,cursor:"pointer",fontFamily:"inherit"}}>
                    {cfg.icon} {a}
                  </button>
                );})}
              </div>
            </div>
            <div><FormLabel label="Cargo / Rol" req/><input value={rol} onChange={e=>setRol(e.target.value)} onKeyDown={e=>e.key==="Enter"&&agregar()} placeholder="Ej: Diseñadora Gráfica" style={inp}/></div>
            <div>
              <FormLabel label="Correo electrónico"/>
              <div style={{position:"relative"}}>
                <span style={{position:"absolute",left:10,top:"50%",transform:"translateY(-50%)",fontSize:13,color:"#94A3B8"}}>✉️</span>
              ...
              <div style={{fontSize:9,color:"#94A3B8",marginTop:3}}>Se usará para notificaciones de asignación</div>
            </div>
            <button onClick={agregar}
              style={{padding:"11px",borderRadius:10,border:"none",background:"linear-gradient(135deg,#2B3A5C,#1a2540)",color:"#fff",fontSize:13,fontWeight:700,cursor:"pointer",fontFamily:"inherit",marginTop:4}}>
              ➕ Agregar al Equipo
            </button>

            {/* Resumen */}
            <div style={{marginTop:8,padding:"12px",background:"#F8FAFC",borderRadius:10,border:"1px solid #E2E8F0"}}>
              <div style={{fontSize:10,color:"#94A3B8",fontWeight:700,letterSpacing:.6,marginBottom:10}}>RESUMEN POR ÁREA</div>
              {AREAS_PROD.map(a=>{
                const cfg=AREA_CFG[a]||{color:"#64748B",icon:"📋"};
                const activos=equipo.filter(m=>m.area===a&&m.activo).length;
                const inact=equipo.filter(m=>m.area===a&&!m.activo).length;
                return(
                  <div key={a} style={{display:"flex",alignItems:"center",gap:8,marginBottom:7}}>
                    <span style={{fontSize:16}}>{cfg.icon}</span>
                    <span style={{fontSize:12,color:"#374151",flex:1,fontWeight:600}}>{a}</span>
                    <span style={{fontSize:11,color:cfg.color,fontWeight:700}}>{activos} activo{activos!==1?"s":""}</span>
                    {inact>0&&<span style={{fontSize:10,color:"#94A3B8"}}>· {inact} inact.</span>}
                  </div>
                );
              })}
            </div>
          ...
          {/* RIGHT: Lista de integrantes */}
          <div style={{padding:"16px 20px",overflow:"auto",display:"flex",flexDirection:"column",gap:14}}>
            {AREAS_PROD.map(a=>{
              const miembros=equipo.filter(m=>m.area===a);
              if(!miembros.length) return null;
              const cfg=AREA_CFG[a]||{color:"#64748B",icon:"📋",bg:"#F8FAFC",border:"#E2E8F0"};
              return(
                <div key={a}>
                  <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:8}}>
                    <span style={{fontSize:14}}>{cfg.icon}</span>
                    <span style={{fontSize:11,fontWeight:800,color:cfg.color,letterSpacing:.4}}>{a.toUpperCase()}</span>
                    <span style={{fontSize:10,color:"#94A3B8"}}>({miembros.length})</span>
                  </div>
                  <div style={{display:"flex",flexDirection:"column",gap:6}}>
                    {miembros.map(m=>(
                      <div key={m.id} style={{display:"flex",alignItems:"center",gap:10,padding:"10px 12px",borderRadius:10,background:m.activo?"#fff":"#F8FAFC",border:`1px solid ${m.activo?cfg.border||cfg.color+"22":"#E2E8F0"}`,opacity:m.activo?1:.7}}>
                        {/* Avatar */}
                        <div style={{width:36,height:36,borderRadius:"50%",background:m.activo?`linear-gradient(135deg,${m.color},${m.color}99)`:"#E2E8F0",display:"flex",alignItems:"center",justifyContent:"center",fontSize:12,fontWeight:800,color:"#fff",flexShrink:0}}>
                          {m.avatar}
                        </div>
                        {/* Info */}
                        <div style={{flex:1,minWidth:0}}>
                          <div style={{fontSize:13,fontWeight:700,color:m.activo?"#0F172A":"#94A3B8"}}>{m.nombre}</div>
                          <div style={{fontSize:10,color:"#94A3B8"}}>{m.rol}</div>
                          {m.email&&<div style={{fontSize:10,color:"#4A90C4",marginTop:1,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>✉️ {m.email}</div>}
                        </div>
                        {/* Estado toggle */}
                        <div style={{display:"flex",alignItems:"center",gap:8}}>
                          <div style={{textAlign:"center"}}>
                            <button onClick={()=>toggleActivo(m.id)}
                              style={{padding:"3px 9px",borderRadius:20,border:"none",background:m.activo?"#F0FDF4":"#F8FAFC",color:m.activo?"#3BAD6E":"#94A3B8",fontSize:10,fontWeight:700,cursor:"pointer",fontFamily:"inherit",border:`1px solid ${m.activo?"#A7F3D0":"#E2E8F0"}`}}>
                              {m.activo?"● Activo":"○ Inactivo"}
                            </button>
                          </div>
                          <button onClick={()=>setConfirmElim(m.id)}
                            style={{width:28,height:28,borderRadius:7,background:"transparent",border:"1px solid #FECACA",color:"#FECACA",cursor:"pointer",fontSize:13,display:"flex",alignItems:"center",justifyContent:"center",transition:"all .14s",flexShrink:0}}
                            onMouseEnter={e=>{e.currentTarget.style.background="#FEF2F2";e.currentTarget.style.color="#E84B2C";}}
                            onMouseLeave={e=>{e.currentTarget.style.background="transparent";e.currentTarget.style.color="#FECACA";}}>
                            🗑
                          </button>
                        </div>
                      </div>
        ...
        <div style={{padding:"12px 24px",borderTop:"1px solid #E2E8F0",background:"#FAFAFA",flexShrink:0,display:"flex",justifyContent:"flex-end"}}>
          <button onClick={onClose} style={{padding:"9px 22px",borderRadius:10,border:"1px solid #E2E8F0",background:"#fff",color:"#64748B",cursor:"pointer",fontFamily:"inherit",fontSize:13,fontWeight:600}}>Cerrar</button>
        </div>
      </div>
    </div>
  );
...
function ModalNueva({ onClose, onSave, notify }) {
  ...
  const [meta, setMeta] = useState({
    tipo:"Webinar", solicitante:"", email:"",
    area_solicitante:"Eventos y Proyectos", proyecto:"Atrévete a Emprender",
    urgente:false, justificacion:"", fecha_inicio: todayStr(),
  });
  const [datos, setDatos] = useState({});
  const [dlOverrides, setDlOverrides] = useState({});
  const [removedIds, setRemovedIds] = useState(new Set());
  const [extraSTs, setExtraSTs] = useState([]);
  const [newST, setNewST] = useState({area:"Diseño",tipo:"",esRedes:false,esData:false,deadline:""});
  const [showAddST, setShowAddST] = useState(false);

  const um=(k,v)=>setMeta(p=>({...p,[k]:v}));
  const ud=(k,v)=>setDatos(p=>({...p,[k]:v}));
  const tipoCfg=tc(meta.tipo);
  const tpl=SUBTAREAS_TPL[meta.tipo]||[];

  const subtareasBase = useMemo(()=>tpl.map((t,i)=>({
    ...t, _id:`b${i}`,
    deadline: dlOverrides[`b${i}`]!==undefined ? dlOverrides[`b${i}`] : (t.dep?"":addBizDays(meta.fecha_inicio,t.dias)),
  })),[meta.tipo, meta.fecha_inicio, dlOverrides]);

  const activeBase  = subtareasBase.filter(t=>!removedIds.has(t._id));
  const activePreview = [...activeBase, ...extraSTs];
  const areasActive = [...new Set(activePreview.map(t=>t.area||"Sin área"))];

  const addExtraST=()=>{
    if(!newST.tipo.trim()){notify("Escribe el nombre de la subtarea","warn");return;}
    const _id=`e${Date.now()}`;
    setExtraSTs(p=>[...p,{...newST,_id,dias:0,dep:null,depDesc:null}]);
    setNewST({area:"Diseño",tipo:"",esRedes:false,esData:false,deadline:""});
    setShowAddST(false);notify("Subtarea añadida ✓");
  };
  const removeBase=(id)=>setRemovedIds(p=>new Set([...p,id]));
  const removeExtra=(id)=>setExtraSTs(p=>p.filter(t=>t._id!==id));

  const guardar=()=>{
    if(!meta.solicitante.trim()){notify("El solicitante es requerido","warn");return;}
    if(!datos.titulo?.trim()){notify("El título/nombre es requerido","warn");return;}
    if(!activePreview.length){notify("Debe haber al menos una subtarea","warn");return;}
    const ts_creacion=nowTs();
    const subtareas=activePreview.map((t,i)=>({
      id:Date.now()+i+Math.random(),
      area:t.area, tipo:t.tipo, esRedes:t.esRedes||false,
      estado:"Pendiente",
      deadline:dlOverrides[t._id]!==undefined?dlOverrides[t._id]:(t.deadline||""),
      diasBase:t.dias||0,
      dep:t.dep||null, depDesc:t.depDesc||null,
      entregaUrl:null, aprobado:null, comentario:null, notas:[],
      historial:[{ts:ts_creacion,tipo:"creada",autor:"Director",detalle:null}],
      asignadoId:null, esData:t.esData||false,
      ts_enviado:null, ts_aprobado:null, ts_rechazado:null, ts_publicado:null, ts_cambios:null,
    }));
    onSave({id:Date.now(),refCode:genRef(),ts_creacion,ts_completado:null,estado:"activa",tipo:meta.tipo,proyecto:meta.proyecto,
      solicitante:meta.solicitante,email:meta.email,area_solicitante:meta.area_solicitante,
      urgente:meta.urgente,datos,notas:[],subtareas});
    onClose();
    notify(`✅ Solicitud creada con ${subtareas.length} subtareas`);
  };

  const inp = INP_STYLE;
  const areasEnTipo=[...new Set(tpl.map(t=>t.area).filter(Boolean))];
  ...
  return(
    <div style={{position:"fixed",inset:0,background:"rgba(15,23,42,.65)",zIndex:700,display:"flex",alignItems:"center",justifyContent:"center",backdropFilter:"blur(4px)"}}>
      <div style={{background:"#fff",borderRadius:20,width:680,maxHeight:"94vh",overflow:"hidden",boxShadow:"0 28px 70px rgba(0,0,0,.28)",display:"flex",flexDirection:"column"}}>
        <div style={{padding:"16px 24px",background:"#2B3A5C",borderBottom:"3px solid #E84B2C",flexShrink:0}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            <div>
              <div style={{fontFamily:"Syne,sans-serif",fontSize:16,fontWeight:800,color:"#fff"}}>Nueva Solicitud</div>
              <div style={{fontSize:11,color:"rgba(255,255,255,.5)",marginTop:1}}>{["Paso 1/3 — Tipo y datos generales","Paso 2/3 — Información del contenido","Paso 3/3 — Revisar deadlines"][step]}</div>
            </div>
            <div style={{display:"flex",alignItems:"center",gap:10}}>
              <div style={{display:"flex",gap:4}}>{[0,1,2].map(i=><div key={i} style={{width:22,height:4,borderRadius:2,background:step>=i?"#E84B2C":"rgba(255,255,255,.2)"}}/>)}</div>
              <button onClick={onClose} style={{width:28,height:28,borderRadius:"50%",background:"rgba(255,255,255,.15)",border:"none",color:"#fff",cursor:"pointer",fontSize:18,display:"flex",alignItems:"center",justifyContent:"center"}}>×</button>
            </div>
          </div>
        </div>
        ...
        <div style={{flex:1,overflow:"auto",padding:"20px 24px"}}>
          {step===0&&(
            <div style={{display:"flex",flexDirection:"column",gap:16}}>
              <div>
                <FormLabel label="Tipo de solicitud" req/>
                <div style={{display:"flex",gap:7,flexWrap:"wrap",marginTop:4}}>
                  {TIPOS.map(t=>{const cfg=tc(t);const isA=meta.tipo===t;return(
                    <button key={t} onClick={()=>{um("tipo",t);setDlOverrides({});}}
                      style={{padding:"7px 13px",borderRadius:8,background:isA?cfg.bg:"#F8FAFC",border:`2px solid ${isA?cfg.color:"#E2E8F0"}`,color:isA?cfg.color:"#374151",fontSize:12,fontWeight:isA?700:500,cursor:"pointer",fontFamily:"inherit",transition:"all .14s"}}>
                      {cfg.icon} {t}
                    </button>
                  );})}
                </div>
              </div>
              <div style={{padding:"12px 16px",background:tipoCfg.bg,border:`1px solid ${tipoCfg.border}`,borderRadius:12,display:"flex",justifyContent:"space-between",alignItems:"center",gap:12}}>
                <div>
                  <div style={{fontSize:13,fontWeight:700,color:tipoCfg.color,marginBottom:4}}>{tipoCfg.icon} {meta.tipo}</div>
                  <div style={{fontSize:11,color:"#64748B"}}>Se generarán <strong style={{color:tipoCfg.color}}>{tpl.length} subtareas</strong> en:</div>
                  <div style={{display:"flex",gap:5,marginTop:5,flexWrap:"wrap"}}>
                    {areasEnTipo.map(a=>{const cfg=ac(a);return <Badge key={a} color={cfg.color} bg={cfg.bg} border={cfg.border} sm>{cfg.icon} {a}</Badge>;})}
                  </div>
                </div>
                <div style={{fontSize:40,opacity:.25}}>{tipoCfg.icon}</div>
              </div>
              <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(200px,1fr))",gap:12}}>
                <div><FormLabel label="Solicitante" req/><input value={meta.solicitante} onChange={e=>um("solicitante",e.target.value)} placeholder="Nombre completo" style={inp}/></div>
                <div><FormLabel label="Email"/><input value={meta.email} onChange={e=>um("email",e.target.value)} placeholder="email@castalatina.org" style={inp}/></div>
                <div><FormLabel label="Área del solicitante"/>
                  <select value={meta.area_solicitante} onChange={e=>um("area_solicitante",e.target.value)} style={inp}>
                    {AREAS_EQUIPO.map(a=><option key={a}>{a}</option>)}
                  </select>
                </div>
                <div><FormLabel label="Proyecto"/>
                  <select value={meta.proyecto} onChange={e=>um("proyecto",e.target.value)} style={inp}>
                    {PROYECTOS.map(p=><option key={p}>{p}</option>)}
                  </select>
                </div>
                <div>
                  <FormLabel label="Fecha de inicio" req/>
                  <input type="date" value={meta.fecha_inicio} onChange={e=>{um("fecha_inicio",e.target.value);setDlOverrides({});}} style={inp}/>
                </div>
                <div style={{display:"flex",alignItems:"flex-end",paddingBottom:2}}>
                  <label onClick={()=>um("urgente",!meta.urgente)} style={{display:"flex",alignItems:"center",gap:10,cursor:"pointer"}}>
                    <div style={{width:40,height:22,borderRadius:11,background:meta.urgente?"#E84B2C":"#E2E8F0",position:"relative",transition:"background .2s"}}>
                      <div style={{position:"absolute",top:3,left:meta.urgente?21:3,width:16,height:16,borderRadius:"50%",background:"#fff",transition:"left .2s",boxShadow:"0 1px 3px rgba(0,0,0,.2)"}}/>
                    </div>
                    <span style={{fontSize:13,fontWeight:600,color:meta.urgente?"#E84B2C":"#64748B"}}>⚡ Urgente</span>
                  </label>
                </div>
                {meta.urgente&&<div style={{gridColumn:"1/-1"}}>
                  <FormLabel label="Justificación de urgencia"/>
                  <textarea value={meta.justificacion} onChange={e=>um("justificacion",e.target.value)} placeholder="¿Por qué no puede seguir los tiempos estándar?" style={{...inp,height:68,resize:"none"}}/>
                </div>}
              </div>
            </div>
          )}
          {step===1&&(
            <div style={{display:"flex",flexDirection:"column",gap:14}}>
              <div style={{padding:"10px 14px",background:tipoCfg.bg,border:`1px solid ${tipoCfg.border}`,borderRadius:10,display:"flex",alignItems:"center",gap:8}}>
                <span style={{fontSize:20}}>{tipoCfg.icon}</span>
                <div>
                  <div style={{fontSize:12,fontWeight:700,color:tipoCfg.color}}>{meta.tipo}</div>
                  <div style={{fontSize:11,color:"#64748B"}}>Información adjunta en las {tpl.length} subtareas</div>
                </div>
              </div>
              <FormTipo tipo={meta.tipo} datos={datos} ud={ud} inp={inp}/>
            </div>
          )}
          {step===2&&(
            <div style={{display:"flex",flexDirection:"column",gap:10}}>
              {/* Cabecera con contador y botón agregar */}
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"10px 14px",background:"#EEF2FF",border:"1px solid #C7D2FE",borderRadius:10}}>
                <span style={{fontSize:12,fontWeight:700,color:"#3730A3"}}>
                  📋 {activePreview.length} subtarea{activePreview.length!==1?"s":""} · {areasActive.length} área{areasActive.length!==1?"s":""}
                </span>
                <button onClick={()=>setShowAddST(p=>!p)}
                  style={{padding:"5px 14px",borderRadius:8,background:showAddST?"#E2E8F0":"linear-gradient(135deg,#2B3A5C,#1a2540)",color:showAddST?"#64748B":"#fff",border:"none",fontSize:11,fontWeight:700,cursor:"pointer",fontFamily:"inherit"}}>
                  {showAddST?"✕ Cancelar":"➕ Agregar subtarea"}
                </button>
              </div>

              {/* Form agregar subtarea personalizada */}
              {showAddST&&(
                <div style={{padding:"14px 16px",background:"#F8FAFC",border:"1px dashed #C7D2FE",borderRadius:10,display:"flex",flexDirection:"column",gap:10}}>
                  <div style={{fontSize:10,color:"#3730A3",fontWeight:800,letterSpacing:.5}}>NUEVA SUBTAREA PERSONALIZADA</div>
                  <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(160px,1fr))",gap:8}}>
                    <div>
                      <div style={{fontSize:9,color:"#64748B",fontWeight:700,marginBottom:3}}>ÁREA *</div>
                      <select value={newST.area} onChange={e=>setNewST(p=>({...p,area:e.target.value}))}
                        style={{width:"100%",padding:"7px 10px",borderRadius:7,border:"1px solid #E2E8F0",fontSize:12,fontFamily:"inherit",outline:"none"}}>
                        {AREAS_PROD.map(a=><option key={a}>{a}</option>)}
                      </select>
                    </div>
                    <div>
                      <div style={{fontSize:9,color:"#64748B",fontWeight:700,marginBottom:3}}>NOMBRE *</div>
                      <input value={newST.tipo} onChange={e=>setNewST(p=>({...p,tipo:e.target.value}))}
                        onKeyDown={e=>e.key==="Enter"&&addExtraST()}
                        placeholder="Ej: Diseño banner principal..."
                        style={{width:"100%",padding:"7px 10px",borderRadius:7,border:"1px solid #E2E8F0",fontSize:12,fontFamily:"inherit",outline:"none"}}/>
                    </div>
                    <div>
                      <div style={{fontSize:9,color:"#64748B",fontWeight:700,marginBottom:3}}>DEADLINE</div>
                      <input type="date" value={newST.deadline} onChange={e=>setNewST(p=>({...p,deadline:e.target.value}))}
                        style={{width:"100%",padding:"7px 10px",borderRadius:7,border:"1px solid #E2E8F0",fontSize:12,fontFamily:"inherit",outline:"none"}}/>
                    </div>
                  </div>
                  <div style={{display:"flex",gap:14,flexWrap:"wrap",alignItems:"center"}}>
                    <label style={{display:"flex",alignItems:"center",gap:5,cursor:"pointer",fontSize:11,color:"#374151"}}>
                      <input type="checkbox" checked={newST.esRedes} onChange={e=>setNewST(p=>({...p,esRedes:e.target.checked}))} style={{accentColor:"#9B59B6"}}/>
                      📱 Redes
                    </label>
                    <label style={{display:"flex",alignItems:"center",gap:5,cursor:"pointer",fontSize:11,color:"#374151"}}>
                      <input type="checkbox" checked={newST.esData} onChange={e=>setNewST(p=>({...p,esData:e.target.checked}))} style={{accentColor:"#0EA5E9"}}/>
                      📊 Data
                    </label>
                    <button onClick={addExtraST}
                      style={{marginLeft:"auto",padding:"7px 20px",borderRadius:8,background:"linear-gradient(135deg,#4338CA,#3730A3)",color:"#fff",border:"none",fontSize:12,fontWeight:700,cursor:"pointer",fontFamily:"inherit"}}>
                      ➕ Agregar
                    </button>
                  </div>
                </div>
              ...
              {/* Subtareas agrupadas por área */}
              {areasActive.map(area=>{
                const tareas=activePreview.filter(t=>(t.area||"Sin área")===area);
                if(!tareas.length) return null;
                const cfg=ac(area);
                return(
                  <div key={area} style={{background:"#fff",border:"1px solid #E2E8F0",borderRadius:12,overflow:"hidden"}}>
                    <div style={{padding:"9px 14px",background:`linear-gradient(90deg,${cfg.bg},#fff)`,borderBottom:"1px solid #F1F5F9",display:"flex",alignItems:"center",gap:8}}>
                      <span style={{fontSize:15}}>{cfg.icon}</span>
                      <span style={{fontSize:12,fontWeight:800,color:cfg.color}}>{area}</span>
                      <Badge color={cfg.color} bg={cfg.bg} sm>{tareas.length}</Badge>
                    </div>
                    <table style={{width:"100%",borderCollapse:"collapse"}}>
                      <thead><tr style={{background:"#F8FAFC"}}>
                        <th style={{padding:"6px 14px",textAlign:"left",fontSize:9,color:"#94A3B8",fontWeight:700,letterSpacing:.6}}>TAREA</th>
                        <th style={{padding:"6px 10px",textAlign:"left",fontSize:9,color:"#94A3B8",fontWeight:700,letterSpacing:.6}}>BASE</th>
                        <th style={{padding:"6px 10px",textAlign:"left",fontSize:9,color:"#94A3B8",fontWeight:700,letterSpacing:.6}}>DEADLINE</th>
                        <th style={{padding:"6px 8px",width:28}}></th>
                      </tr></thead>
                      <tbody>
                        {tareas.map(t=>(
                          <tr key={t._id||t.id} style={{borderTop:"1px solid #F1F5F9"}}>
                            <td style={{padding:"8px 14px"}}>
                              <div style={{fontSize:12,fontWeight:600,color:"#0F172A"}}>{t.tipo}</div>
                              {t.depDesc&&<div style={{fontSize:9,color:"#E84B2C",marginTop:1}}>⛓ {t.depDesc}</div>}
                              <div style={{display:"flex",gap:4,marginTop:1}}>
                                {t.esRedes&&<span style={{fontSize:8,color:"#9B59B6",fontWeight:700}}>📱</span>}
                                {t.esData&&<span style={{fontSize:8,color:"#0EA5E9",fontWeight:700}}>📊</span>}
                                {t._id&&t._id.startsWith("e")&&<span style={{fontSize:8,color:"#8B5CF6",background:"#F5F3FF",padding:"0 4px",borderRadius:3,fontWeight:700}}>+ personalizada</span>}
                              </div>
                            </td>
                            <td style={{padding:"8px 10px",fontSize:10,color:"#94A3B8",whiteSpace:"nowrap"}}>
                              {t.dep?<span style={{color:"#E84B2C",fontSize:10}}>Dep.</span>:t.dias?`${t.dias}d`:"—"}
                            </td>
                            <td style={{padding:"8px 10px"}}>
                              <input type="date"
                                value={dlOverrides[t._id]!==undefined?dlOverrides[t._id]:(t.deadline||"")}
                                onChange={e=>setDlOverrides(p=>({...p,[t._id||t.id]:e.target.value}))}
                                style={{padding:"4px 6px",borderRadius:6,border:`1px solid ${(dlOverrides[t._id]||t.deadline)?"#3BAD6E":"#FECACA"}`,fontSize:11,color:"#1E293B",outline:"none",fontFamily:"inherit",background:(dlOverrides[t._id]||t.deadline)?"#F0FDF4":"#FEF2F2"}}/>
                            </td>
                            <td style={{padding:"8px",textAlign:"center"}}>
                              <button
                                onClick={()=>t._id&&t._id.startsWith("e")?removeExtra(t._id):removeBase(t._id||String(t.id))}
                                title="Eliminar"
                                style={{width:22,height:22,borderRadius:5,background:"#FEF2F2",border:"1px solid #FECACA",color:"#EF4444",cursor:"pointer",fontSize:13,display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"inherit",lineHeight:1}}>
                                ×
                              </button>
                            </td>
                          </tr>
              ...
              {activePreview.length===0&&(
                <div style={{textAlign:"center",padding:"24px",color:"#94A3B8",fontSize:12,background:"#fff",borderRadius:10,border:"1px dashed #E2E8F0"}}>
                  Sin subtareas activas. Agrega al menos una para continuar.
                </div>
              )}
            </div>
          )}
        ...
        <div style={{padding:"13px 24px",borderTop:"1px solid #E2E8F0",display:"flex",justifyContent:"space-between",flexShrink:0,background:"#FAFAFA"}}>
          <button onClick={step===0?onClose:()=>setStep(s=>s-1)} style={{padding:"10px 20px",borderRadius:10,border:"1px solid #E2E8F0",background:"#fff",color:"#64748B",cursor:"pointer",fontFamily:"inherit",fontSize:13,fontWeight:600}}>
            {step===0?"Cancelar":"← Atrás"}
          </button>
          {step<2
            ?<button onClick={()=>{if(step===0&&!meta.solicitante.trim()){notify("Solicitante requerido","warn");return;}setStep(s=>s+1);}} style={{padding:"10px 24px",borderRadius:10,border:"none",background:"linear-gradient(135deg,#2B3A5C,#1a2540)",color:"#fff",cursor:"pointer",fontFamily:"inherit",fontSize:13,fontWeight:700}}>Continuar →</button>
            :<button onClick={guardar} style={{padding:"10px 24px",borderRadius:10,border:"none",background:"linear-gradient(135deg,#E84B2C,#C0392B)",color:"#fff",cursor:"pointer",fontFamily:"inherit",fontSize:13,fontWeight:700,boxShadow:"0 4px 14px rgba(232,75,44,.3)"}}>✓ Crear {activePreview.length} Subtareas</button>
          }
        </div>
      </div>
    </div>
...
// ─── FORM DATOS POR TIPO ──────────────────────────────────────────────────────
function FormTipo({tipo,datos,ud,inp}){
  const ta=(k,ph,req)=>(<div key={k} style={{gridColumn:"1/-1"}}><FormLabel label={ph} req={req}/><textarea value={datos[k]||""} onChange={e=>ud(k,e.target.value)} placeholder={ph} style={{...inp,height:88,resize:"none"}}/></div>);
  const tx=(k,ph,req,wide)=>(<div key={k} style={wide?{gridColumn:"1/-1"}:{}}><FormLabel label={ph} req={req}/><input value={datos[k]||""} onChange={e=>ud(k,e.target.value)} placeholder={ph} style={inp}/></div>);
  const lk=(k,ph)=>(<div key={k} style={{gridColumn:"1/-1"}}><FormLabel label={ph}/><input value={datos[k]||""} onChange={e=>ud(k,e.target.value)} placeholder="https://" style={{...inp,color:"#E84B2C"}}/></div>);
  const se=(k,ph,opts)=>(<div key={k}><FormLabel label={ph}/><select value={datos[k]||""} onChange={e=>ud(k,e.target.value)} style={inp}><option value="">Seleccionar...</option>{opts.map(o=><option key={o}>{o}</option>)}</select></div>);
  const W=FormGrid;
  if(tipo==="Webinar") return <W>{tx("titulo","Título del webinar",true,true)}{tx("fecha","Fecha y hora (Ej: 2026-04-15 19:00 ET)")}{ta("descripcion","Descripción del webinar",true)}{lk("link_zoom","Link de Zoom / Registro")}{tx("host","Nombre del Host")}{tx("invitado1","Invitado 1 — Nombre y cargo",true)}{lk("foto_invitado1","Link foto Invitado 1 (Drive)")}{tx("invitado2","Invitado 2 (opcional)")}{lk("carpeta","📁 Carpeta de Drive")}</W>;
  if(tipo==="Diseño Gráfico") return <W>{tx("titulo","Nombre / descripción de la pieza",true,true)}{ta("objetivo","Objetivo y mensaje principal",true)}{ta("texto","Texto exacto que debe incluir",true)}{lk("carpeta","📁 Carpeta de Drive")}</W>;
  if(tipo==="Video / Reel / Cápsula") return <W>{tx("titulo","Nombre del video/reel",true,true)}{ta("descripcion","Descripción del contenido",true)}{se("requiere_grabacion","¿Requiere grabación nueva?",["Sí, grabación nueva","No, material existente","No estoy seguro"])}<div/>{lk("carpeta","📁 Carpeta de Drive con material base")}</W>;
  if(tipo==="Artículo / Blog") return <W>{tx("titulo","Título tentativo del artículo",true,true)}{ta("descripcion","Descripción y enfoque",true)}{lk("carpeta","📁 Carpeta de Drive")}</W>;
  if(tipo==="Redes Sociales") return <W>{tx("titulo","Nombre/descripción del contenido",true,true)}{tx("canales","Canales de publicación")}{ta("descripcion","¿Qué comunicar? Objetivo y audiencia",true)}{lk("carpeta","📁 Carpeta de Drive")}</W>;
  if(tipo==="Página Web") return <W>{tx("titulo","Nombre de la página",true,true)}{se("tipo_web","Tipo de solicitud",["Página nueva","Actualización","Landing page","Ajuste de contenido"])}{ta("descripcion","Descripción detallada",true)}{lk("carpeta","📁 Carpeta de Drive")}</W>;
  if(tipo==="Evento Presencial") return <W>{tx("titulo","Nombre del Evento",true,true)}{tx("fecha","Fecha y hora del Evento")}{tx("ciudad","Ciudad y dirección")}{ta("descripcion","Descripción general",true)}{lk("carpeta","📁 Carpeta de Drive")}</W>;
  return <W>{tx("titulo","Título de la solicitud",true,true)}{ta("descripcion","Describe tu solicitud con detalle",true)}{lk("carpeta","📁 Carpeta de Drive (opcional)")}</W>;
}
...
function ModalAgregarSubtarea({ sol, onClose, onAdd, notify }) {
  ...
  const guardar = () => {
    if (!tipo.trim()) { notify("Escribe el tipo/nombre de la subtarea","warn"); return; }
    const ts = nowTs();
    onAdd({
      id: Date.now(),
      area, tipo, esRedes, esData,
      estado: "Pendiente",
      deadline, diasBase: null,
      dep: null, depDesc: null,
      entregaUrl: null, aprobado: null, comentario: null, notas:[],
      historial: [{ ts, tipo:"creada", autor:"Director", detalle:"Agregada manualmente" }],
      asignadoId:null,
      ts_enviado:null, ts_aprobado:null, ts_rechazado:null, ts_publicado:null, ts_cambios:null,
    });
    notify(`✅ Subtarea "${tipo}" agregada`);
    onClose();
  };
  ...
  return (
    <div style={{position:"fixed",inset:0,background:"rgba(15,23,42,.6)",zIndex:800,display:"flex",alignItems:"center",justifyContent:"center",backdropFilter:"blur(4px)"}}>
      <div style={{background:"#fff",borderRadius:18,width:440,overflow:"hidden",boxShadow:"0 28px 70px rgba(0,0,0,.25)"}}>
        <div style={{padding:"16px 22px",background:"#2B3A5C",borderBottom:"3px solid #E84B2C"}}>
          <div style={{fontFamily:"Syne,sans-serif",fontSize:15,fontWeight:800,color:"#fff"}}>➕ Agregar Subtarea</div>
          <div style={{fontSize:11,color:"rgba(255,255,255,.5)",marginTop:1}}>para "{sol.datos.titulo||sol.tipo}"</div>
        </div>
        <div style={{padding:"20px 22px",display:"flex",flexDirection:"column",gap:14}}>
          <div>
            <FormLabelSimple label="Área"/>
            <div style={{display:"flex",gap:6,flexWrap:"wrap",marginTop:4}}>
              {AREAS_PROD.map(a=>{const cfg=ac(a);const isA=area===a;return(
                <button key={a} onClick={()=>setArea(a)}
                  style={{padding:"6px 12px",borderRadius:7,border:`2px solid ${isA?cfg.color:"#E2E8F0"}`,background:isA?cfg.bg:"#fff",color:isA?cfg.color:"#64748B",fontSize:12,fontWeight:isA?700:400,cursor:"pointer",fontFamily:"inherit"}}>
                  {cfg.icon} {a}
                </button>
              );})}
            </div>
          ...
          <div>
            <FormLabelSimple label="Nombre / tipo de tarea"/>
            <input value={tipo} onChange={e=>setTipo(e.target.value)} onKeyDown={e=>e.key==="Enter"&&guardar()} placeholder="Ej: Banner newsletter, Copy email..." style={inp}/>
          </div>
          <div>
            <FormLabelSimple label="Deadline"/>
            <input type="date" value={deadline} onChange={e=>setDeadline(e.target.value)} style={inp}/>
          </div>
          <label onClick={()=>setEsRedes(p=>!p)} style={{display:"flex",alignItems:"center",gap:10,cursor:"pointer",padding:"8px 10px",background:esRedes?"#F5F0FB":"#F8FAFC",borderRadius:8,border:`1px solid ${esRedes?"#D7BDE2":"#E2E8F0"}`}}>
            <div style={{width:36,height:20,borderRadius:10,background:esRedes?"#9B59B6":"#E2E8F0",position:"relative",transition:"background .2s",flexShrink:0}}>
              <div style={{position:"absolute",top:2,left:esRedes?18:2,width:16,height:16,borderRadius:"50%",background:"#fff",transition:"left .2s"}}/>
            </div>
          ...
          <label onClick={()=>setEsData(p=>!p)} style={{display:"flex",alignItems:"center",gap:10,cursor:"pointer",padding:"8px 10px",background:esData?"#F0F9FF":"#F8FAFC",borderRadius:8,border:`1px solid ${esData?"#BAE6FD":"#E2E8F0"}`}}>
            <div style={{width:36,height:20,borderRadius:10,background:esData?"#0EA5E9":"#E2E8F0",position:"relative",transition:"background .2s",flexShrink:0}}>
              <div style={{position:"absolute",top:2,left:esData?18:2,width:16,height:16,borderRadius:"50%",background:"#fff",transition:"left .2s"}}/>
            </div>
            <span style={{fontSize:12,color:esData?"#0EA5E9":"#64748B",fontWeight:esData?700:400}}>📊 Esta tarea es un reporte de Data (métricas y análisis)</span>
          </label>
        </div>
        <div style={{padding:"0 22px 18px",display:"flex",gap:8,justifyContent:"flex-end"}}>
          <button onClick={onClose} style={{padding:"9px 18px",borderRadius:9,border:"1px solid #E2E8F0",background:"#fff",color:"#64748B",cursor:"pointer",fontFamily:"inherit",fontSize:13,fontWeight:600}}>Cancelar</button>
          <button onClick={guardar} style={{padding:"9px 20px",borderRadius:9,border:"none",background:"linear-gradient(135deg,#2B3A5C,#1a2540)",color:"#fff",cursor:"pointer",fontFamily:"inherit",fontSize:13,fontWeight:700}}>➕ Agregar Subtarea</button>
        </div>
      </div>
    </div>
  );
...
function ConfirmDialog({ title, msg, onConfirm, onCancel, danger }) {
  return (
    <div style={{position:"fixed",inset:0,background:"rgba(15,23,42,.6)",zIndex:900,display:"flex",alignItems:"center",justifyContent:"center",backdropFilter:"blur(4px)"}}>
      <div style={{background:"#fff",borderRadius:16,width:380,overflow:"hidden",boxShadow:"0 20px 60px rgba(0,0,0,.25)"}}>
        <div style={{padding:"18px 22px",borderBottom:`3px solid ${danger?"#E84B2C":"#E2E8F0"}`}}>
          <div style={{fontSize:16,fontWeight:800,fontFamily:"Syne,sans-serif",color:"#0F172A"}}>{title}</div>
        </div>
        <div style={{padding:"16px 22px",fontSize:13,color:"#64748B",lineHeight:1.6}}>{msg}</div>
        <div style={{padding:"0 22px 18px",display:"flex",gap:8,justifyContent:"flex-end"}}>
          <button onClick={onCancel} style={{padding:"9px 18px",borderRadius:9,border:"1px solid #E2E8F0",background:"#fff",color:"#64748B",cursor:"pointer",fontFamily:"inherit",fontSize:13,fontWeight:600}}>Cancelar</button>
          <button onClick={onConfirm} style={{padding:"9px 20px",borderRadius:9,border:"none",background:danger?"linear-gradient(135deg,#E84B2C,#C0392B)":"linear-gradient(135deg,#2B3A5C,#1a2540)",color:"#fff",cursor:"pointer",fontFamily:"inherit",fontSize:13,fontWeight:700}}>
...
function ModalDetalle({sol,usuario,equipo=[],onClose,onUpdSol,onDeleteSol,notify}){
  ...
  return(
    <div style={{position:"fixed",inset:0,background:"rgba(15,23,42,.6)",zIndex:600,display:"flex",alignItems:"center",justifyContent:"center",backdropFilter:"blur(4px)"}}>
      ...
      <div style={{background:"#FAFAFA",borderRadius:20,width:"min(1160px,94vw)",height:"min(90vh,90dvh)",overflow:"hidden",boxShadow:"0 28px 70px rgba(0,0,0,.25)",display:"flex",flexDirection:"column"}}>
        {/* Header */}
  ...
  return(
    <div style={{background:"#fff",borderRadius:14,border:`1.5px solid ${open?cfg.color:"#E8ECF4"}`,overflow:"hidden",boxShadow:open?`0 6px 24px ${cfg.color}22`:"0 2px 6px rgba(15,23,42,.06)",transition:"all .18s",marginBottom:10}}>
      ...
      <div style={{display:"flex",alignItems:"stretch"}}>
        ...
        <div onClick={()=>setOpen(p=>!p)} style={{flex:1,padding:"14px 16px",cursor:"pointer",minWidth:0}}>
          ...
          <div style={{display:"flex",alignItems:"flex-start",gap:10,marginBottom:8}}>
            ...
            <div style={{flex:1,minWidth:0}}>
              <div style={{fontSize:13,fontWeight:700,color:"#0F172A",lineHeight:1.3,marginBottom:4,wordBreak:"break-word"}}>{st.tipo}</div>
              {/* Badges row */}
            ...
            <span style={{fontSize:12,color:"#CBD5E1",flexShrink:0,marginTop:2}}>{open?"▴":"▾"}</span>
          </div>
    ...
    <div style={{minHeight:"100vh",background:"#F1F5F9",fontFamily:"'DM Sans',sans-serif",color:"#1E293B"}}>
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0}
        ...
        .modal-detalle-w{width:min(860px,96vw)}
...
const solToDb = (s) => ({
  ...
  datos:            s.datos           || {},
  notas:            s.notas           || [],
  subtareas:        s.subtareas       || [],
  ts_creacion:      s.ts_creacion     || new Date().toISOString(),
  ts_completado:    s.ts_completado   || null,
});
...
const dbToSol = (r) => ({
  ...
  ts_creacion:      r.ts_creacion,
  ts_completado:    r.ts_completado,
});

export default function App() {
  const [view, setView]        = useState("dashboard");
  const [sols, setSolsRaw]     = useState(null);   // null = not loaded yet
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const sr = localStorage.getItem("cls_sols");
        const er = localStorage.getItem("cls_equipo");
        const loadedSols   = sr ? JSON.parse(sr) : DEMO_PLAIN();
        const loadedEquipo = er ? JSON.parse(er) : EQUIPO_INICIAL;
        setSolsRaw(loadedSols);
        setEquipoRaw(loadedEquipo);
        if (!sr) {
          localStorage.setItem("cls_sols",   JSON.stringify(DEMO_PLAIN()));
          localStorage.setItem("cls_equipo", JSON.stringify(EQUIPO_INICIAL));
        }
      } catch(e) {
  ...
  // ── Wrapped setters — persist every change ──────────────────────────────
  const setSols = useCallback((updater) => {
    setSolsRaw(prev => {
      const next = typeof updater === "function" ? updater(prev || []) : updater;
      // Schedule storage write outside the render cycle
      setTimeout(() => {
        try {
          localStorage.setItem("cls_sols", JSON.stringify(next));
          flashSync("saved");
        } catch(e) { flashSync("error"); }
      }, 0);
      return next;
    });
  }, [flashSync]);
  ...
  const setEquipo = useCallback((updater) => {
    setEquipoRaw(prev => {
      ...
      setTimeout(() => {
        try {
          localStorage.setItem("cls_equipo", JSON.stringify(next));
          flashSync("saved");
        } catch(e) { flashSync("error"); }
      }, 0);
      return next;
  ...
  const SyncBadge = () => syncStatus ? (
    <div style={{position:"fixed",bottom:60,right:16,zIndex:9001,padding:"6px 14px",borderRadius:8,
      background:syncStatus==="saving"?"#2B3A5C":syncStatus==="saved"?"#3BAD6E":"#E84B2C",
      color:"#fff",fontSize:11,fontWeight:700,boxShadow:"0 2px 10px rgba(0,0,0,.3)",
      display:"flex",alignItems:"center",gap:6}}>
      {syncStatus==="saving"&&<div style={{width:10,height:10,border:"2px solid rgba(255,255,255,.3)",
        borderTop:"2px solid #fff",borderRadius:"50%",animation:"spin .6s linear infinite"}}/>}
      {syncStatus==="saving"?"Guardando...":syncStatus==="saved"?"✓ Guardado":"⚠️ Error"}
    </div>
  ) : null;
